...
function TapDoan({ history, permission }) {
  ...
  useEffect(() => {
    function load() {
      dispatch(fetchStart("TapDoan", "GET", null, "LIST"));
    }
  ...
  const deleteItemAction = (item) => {
    ...
    if (item.isRemove) url = `TapDoan/Remove/${item.id}`;
  ...
  const actionContent = (item) => {
    const editItem =
      permission && permission.edit ? (
        <Link
          to={{
            pathname: `/danh-muc/tap-doan/${item.id}/chinh-sua`,
  ...
  const handleRedirect = () => {
    history.push({
      pathname: "/danh-muc/tap-doan/them-moi",
    });
  };

...
function PhongBan({ permission, history }) {
  ...
  useEffect(() => {
    function load() {
      dispatch(fetchStart("PhongBan", "GET", null, "LIST"));
    }
  ...
  const getListData = (keyword, page, pageSize) => {
    ...
    dispatch(fetchStart(`PhongBan?${param}`, "GET", null, "LIST"));
  };
  /**
   * Tìm kiếm người dùng
   *
  ...
  const deleteItemAction = (item) => {
    ...
    if (item.isRemove) url = `PhongBan/Remove/${item.id}`;
  ...
  const actionContent = (item) => {
    const editItem =
      permission && permission.edit ? (
        <Link
          to={{
            pathname: `/danh-muc/phong-ban/${item.id}/chinh-sua`,
  ...
  const handleRedirect = () => {
    history.push({
      pathname: "/danh-muc/phong-ban/them-moi",
    });
  };
